...
if (!file.exists(here::here("data", "results.rda"))) {
    ...
    save(m1, doubled_sd, coef_data0, cy_summary, 
         file = here::here("data", "results.rda"))
} else {
    load(file = here::here("data", "results.rda"))
}

eb_trust_parl2_parties2_perc <- {{dcpo_input_raw1 %>% 
...
The resulting dataset comprises the thirty-eight OECD countries, each observed in sixteen (Turkey) to forty-three (the United States) consecutive years (mean: `r cy_summary %>% nth(4) %>% round(1)` years, median: `r cy_summary %>% nth(3)` years).
This is much more data than is available from any single source, even for this set of relatively data-rich countries.
The two richest single items available for these cases, the Eurobarometer's questions on trust in national parliaments and in political parties, each cover only `r eb_trust_parl2_parties2_perc`% of these country-years; these data exclude entirely the nine OECD members outside Europe.
