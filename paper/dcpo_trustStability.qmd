...
surveys_ds <- read_csv(here::here("data-raw",
                                  "surveys_diffuse_support.csv"),
                        col_types = "cccc") %>% 
       # filter(group == "eff")
    filter(str_detect(group, "eff"))

dcpo_input_raw <- DCPOtools::dcpo_setup(vars = surveys_ds,
                                        datapath = here::here("..",
                                                              "data", "dcpo_surveys"),
                                        file = here::here("data",
                                                          "dcpo_input_raw.csv"))
...
```{r dcpo_results}
 if (!exists("results_path")) {
   ...
   if (!file.exists(file.path(results_path, paste0("dcpo-", latest, "-1.csv")))) {
     ...
     osf_retrieve_node("82w36") %>% # needs updated for discontent
       osf_ls_files() %>% 
...
countries <- c("Netherlands", "Sweden", "Brazil", "United States",
               "Argentina", "Czechia", "South Korea", "Poland", 
               "China", "Ukraine", "Indonesia", "Russia", 
               "Nigeria", "Jamaica", "Mexico", "Uganda")
...
ggplot(data = c_res, aes(x = year, y = mean)) +
  ...
  coord_cartesian(xlim = c(1985, 2025), ylim = c(0, 1)) +
...
# Research Design and Results {.unnumbered}
