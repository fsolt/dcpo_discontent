...
```{r cpi}
if (!file.exists(here::here("data", "cpi.rda"))) {
    ...
    rio::export(cpi, here("data", "cpi.rda"))
} else {
    cpi <- rio::import(here("data", "cpi.rda"))
}
...
if (!file.exists(here::here("data", "data_combo.rda"))) {
    data_combo <- theta_summary %>% 
        ...
        mutate(federal = as.numeric(country %in% c("Australia", # decentralized = strong
               ...
               corrupt_mean = mean(100 - cpi),
               corrupt_mean_se = sqrt(sum(cpi_se^2))/
                   length(cpi),
               corrupt_diff = (100 - cpi) - corrupt_mean,
               corrupt_diff_se = sqrt(cpi_se^2 + corrupt_mean_se^2)/2) %>% 
        ungroup()
...
if (!file.exists(here::here("data", "results_m1.rda"))) {
    ...
    doubled_sd_m1 <- m1$data %>% 
        ...
        transmute(`.variable` = case_when(name == "gini_mean" ~ 
                  ...
                  sd2 = value) %>% 
        mutate(model = "Without Corruption")
...
```{r m2, include=FALSE}
if (!file.exists(here::here("data", "results_m2.rda"))) {
    m2 <- brm(
        formula = bf(
            mean * 100 | mi(sd * 100) ~ year +
                election +
                ...
                me(gini_mean, gini_mean_se) +
                me(gini_diff, gini_diff_se) +
                me(corrupt_mean, corrupt_mean_se) +
                me(corrupt_diff, corrupt_diff_se) +
                (1 | country) + (1 | year)
        ),
        data = data_combo,
        backend = "cmdstanr",
    ...
    doubled_sd_m2 <- m2$data %>% 
        select(-`mean * 100`, -mean, -sd,
               -country, -ends_with("_se")) %>% 
        summarize(across(everything(), by2sd)) %>% 
        pivot_longer(everything()) %>% 
        transmute(`.variable` = case_when(name == "corrupt_mean" ~ 
                                              "bsp_mecorrupt_meancorrupt_mean_se",
                                          name == "corrupt_diff" ~
                                              "bsp_mecorrupt_diffcorrupt_diff_se",
                                          name == "gini_mean" ~ 
                                              "bsp_megini_meangini_mean_se",
                                          name == "gini_diff" ~
                                              "bsp_megini_diffgini_diff_se",
                  ...
                  var_names = c("Time Trend",
                                ...
                                "Corruption, Diff"),
                  sd2 = value) %>% 
        mutate(model = "With Corruption")
    
    coef_data0_m2 <- m2 %>% 
        tidybayes::gather_draws(`bs?p?_.*`, regex = TRUE) %>% 
        filter(!`.variable`=="b_Intercept") %>% 
        left_join(doubled_sd_m2, by = join_by(.variable)) %>% 
        mutate(model = "With Corruption")
    
    cy_summary_m2 <- m2$data %>%
        count(country) %>%
        pull(n) %>%
        summary()  
    
    save(m2, doubled_sd_m2, coef_data0_m2, cy_summary_m2,
         file = here::here("data", "results_m2.rda"))
} else {
    load(file = here::here("data", "results_m2.rda"))
}

eb_corruption_obs <- dcpo_input_raw1 %>% 
...
The resulting dataset comprises the thirty-eight OECD countries.
When corruption is included in the model, the number of country-years observed ranges from sixteen (Turkey) to twenty-eight (the United States) consecutive years (mean: `r cy_summary_m2 %>% nth(4) %>% round(1)` years, median: `r cy_summary_m2 %>% nth(3)` years).
Excluding corruption permits analysis of even more country-years in many countries, up to forty-three in the United States (mean: `r cy_summary_m1 %>% nth(4) %>% round(1)` years, median: `r cy_summary_m1 %>% nth(3)` years).
The advantage over pooling a single question is clear: even for this set of relatively data-rich countries, the two richest single items available for these cases, the Eurobarometer's questions on trust in national parliaments and in political parties, each provide only provide about half as many country-years for analysis (), 

and these Eurobarometer data naturally entirely exclude the nine OECD members outside Europe.
...
plot_notes <- paste0("Notes: Dots indicate posterior means; whiskers, from thickest to thinnest, describe 80%,\n90%, and 95% credible intervals; shading depicts the posterior probability density function. Number of country-year observations: ",
                     nrow(m2$data), " (with corruption) and ",
                     nrow(m1$data), " (without corruption).")

coef_data0_m1 %>% 
    mutate(std_coef = .value * sd2,
           term = factor(var_names, levels = ordered),
           model = "Without Corruption") %>% 
    bind_rows(coef_data0_m2 %>% 
                  mutate(std_coef = .value * sd2,
                         term = factor(var_names, levels = ordered),
                         model = "With Corruption")) %>% 
    ggplot(aes(y = term, x = std_coef, group = model)) +
    ...
    theme_light() +
    scale_color_manual(values = c("With Corruption" = "blue",
                                  "Without Corruption" = "darkgreen"),
                       name = NULL) +
    scale_fill_manual(values = c("With Corruption" = "blue",
                                 "Without Corruption" = "darkgreen"),
                      name = NULL) +
    xlab(NULL) +
    ylab(NULL) +
