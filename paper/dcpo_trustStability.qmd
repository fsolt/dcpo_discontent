...
Validation tests of a novel latent variable, like any new measure, are crucial [see, e.g., @Hu2023].
@fig-convergent and @fig-construct provide evidence of this measure's validity with tests of convergent validation and construct validation.
...
Here, @fig-convergent offers an 'internal' convergent validation test  the PPD scores are compared to responses to individual source-data survey items that were used to generate them; this provides an .

<!-- Consider validation tests with: -->
<!--     democratic satisfaction -->
<!--     trust in government -->
<!--     CPI from Transparency International -->
...
internal_tscs_plot <- validation_plot(internal_tscs_dat,
  ...
  labs(x = "Public Political Discontent",
...
internal_ts_plot <- validation_plot(internal_ts_dat,
  ...
  annotate("text", x = 2010, y = 10, size = 2,
           label = 'SOM') +
  annotate("text", x = 2000, y = 58, size = 2,
           label = "Public Political Discontent")
...
```{r ext_dat, include=FALSE, eval=FALSE}
...
```{r extval1, fig.cap="Construct Validation: Correlations Between Macrointerest and Other Aspects of Political Engagement", fig.height=3.5, eval=FALSE}
...
On the left, PPD scores are plotted against the percentage of respondents across all country-years who responded "tend not to trust" rather than "tend to trust" on the Eurobarometer's dichotomous question, "How much trust do you have in certain institutions: Political parties?"
The middle panel shows responses to the question with the most data-rich cross-section, "Would you say your country is governed by the will of the people?" in the Gallup's 2005 Voice of the People survey.
Finally, the right panel evaluates how well the macrointerest scores capture change over time by focusing on the item with the largest number of observations for a single country in the source data, which asked respondents to Sweden's SOM surveys, "How much confidence do you have in the way the following institutions and groups do their job: The National Parliament?"
