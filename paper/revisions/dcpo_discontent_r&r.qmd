...
The DCPO model and similar approaches have been used in recent years to model public opinion across countries and over time on a variety of topics, such as policy ideology [@Caughey2019; @Berwick2025], gender egalitarianism [@Woo2023], political interest [@Hu2025b], and support for gay rights [@Woo2025].
The underlying logic is that the probability that an individual responds affirmatively to a survey question is a function of the respondent’s score on the latent trait, in this case, political discontent.
...
LFollowing @Jennings2017, we excluded these questions from the latent-variable estimation, not least to avoid assuming that respondents necessarily identify the current political system of their country with democracy.
...
Across countries and years, and for all three of these survey items, our latent-variable measure of political discontent is strongly and negatively correlated with aggregate positive evaluations of democracy.
...
Survey weights, where available, are incorporated in the source data (and the `DCPOtools` package applies them automatically), which reduces the influence of design differences on the aggregated response shares.
Yet not all available weights yield fully representative samples, and some surveys lack weights entirely.
...
To the extent that such sample-representation differences vary across years for a single country and item---as is typically the case, given that more recent surveys tend to be more fully representative---the country-specific item bias terms will not fully remedy this problem.
The downside is greater measurement uncertainty in the estimates in country-years where the data are relatively rich (via the overall dispersion parameter $\phi$) and potential bias in the estimates where data are more sparse.
