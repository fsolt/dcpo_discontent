...
The DCPO model and similar latent modeling approaches have been used in recent years to model public opinion across countries and over time on a variety of topics, such as policy ideology [@Caughey2019; @Berwick2025], gender egalitarianism [@Woo2023], political interest [@Hu2025b], and support for gay rights [@Woo2025].
...
Third, to allow for the possibility that translation issues or cultural differences result in the same question being interpreted differently in different countries, the model estimates *country-specific bias* parameters that shift the diﬀiculty of all responses for a particular question in a particular country.
...
As a result, we generate estimates of the public's political discontent in all `r res_cy` country-years spanned by the source data, which we call Public Political Discontent (PPD) scores.
...
After estimating public political discontent across a wide range of countries and years, we focus on a group of countries where discontent has attracted particular public and scholarly concern, the advanced democracies of the OECD, in order to demonstrate how the newly constructed PPD dataset can be used to address substantive questions in the literature.
...
Translation issues, cultural differences in response styles, or other idiosyncrasies of a country can render the same survey item not equivalent in its performance there relative to other national contexts; this lack of equivalence is made empirically distinguishable from a difference in discontent by the country's contemporary responses to other included survey items.
...
This is a limitation the model shares with many cross-national survey aggregation strategies, and it underscores the importance of the validation tests presented in Appendix\nobreakspace{}\@ref(sec-app-validation), which assess whether these residual design effects are consequential enough to undermine the resulting estimates.
